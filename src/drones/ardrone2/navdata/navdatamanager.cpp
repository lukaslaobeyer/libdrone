...
void NavdataManager::init(std::string ip, boost::asio::io_service &io_service)
{
	...
	udp::resolver::query query(udp::v4(), ip, to_string(ardrone2::NAVDATA_PORT));
	udp::endpoint receiver_endpoint = *resolver.resolve(query);

	socket = new udp::socket(io_service);
	socket->open(udp::v4());
	socket->bind(udp::endpoint(udp::v4(), ardrone2::NAVDATA_PORT));
...
bool NavdataManager::parseNavdata(char data[], int receivedbytes)
{
	...
	while(position < receivedbytes)
	{
		...
		switch(id)
		{
		case ardrone2::navdata_keys::NAVDATA_DEMO_TAG:
			// Read control state
			...
			break;
		case ardrone2::navdata_keys::NAVDATA_PHYS_MEASURES_TAG:
			...
			break;
		case ardrone2::navdata_keys::NAVDATA_ALTITUDE_TAG:
			...
			break;
		case ardrone2::navdata_keys::NAVDATA_VISION_RAW_TAG:
			...
			break;
		case ardrone2::navdata_keys::NAVDATA_CKS_TAG:
			// Read checksum
